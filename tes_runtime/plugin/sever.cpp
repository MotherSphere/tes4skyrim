...
public:
    ...
    void Run() override {
        ...
        if (g_api.applyImpulse) {
            g_api.applyImpulse(g_api.vm, 0, ref_, dir_[0], dir_[1], dir_[2], kLimbImpulse);
        }
    }
...
void DropLimb(void* actor, void* root, const Part& part, const float* dir) {
    if (!part.limbForm || !g_api.placeAtMe || !g_api.setPosition || !g_api.vm) return;
    void* ref = g_api.placeAtMe(g_api.vm, 0, actor, part.limbForm, 1, false, false);
    if (!ref) {
    ...
    g_api.setPosition(g_api.vm, 0, ref, pos[0], pos[1], pos[2] + 5.0f);
...
bool HitOrigin(void* hitData, void* actor, float* pos) {
    ...
    if (g_api.lookupByHandle) g_api.lookupByHandle(&handle, &attacker);
