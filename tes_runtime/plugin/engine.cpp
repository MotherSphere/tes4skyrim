...
bool ResolveEngine() {
    ...
    Bind(g_api.placeAtMe, "ObjectReference.PlaceAtMe", ids::kPlaceAtMe);
    Bind(g_api.setPosition, "ObjectReference.SetPosition", ids::kSetPosition);
    Bind(g_api.applyImpulse, "ObjectReference.ApplyHavokImpulse", ids::kApplyHavokImpulse);
    Bind(g_api.lookupByHandle, "LookupReferenceByHandle", ids::kLookupByHandle);
...
std::string SidecarDir() {
    return ExeDir() + "\\Data\\SKSE\\Plugins\\TESRuntime";
}
